...
export function CreateDiceModal({ open, onClose, onCreate }: CreateDiceModalProps) {
  ...
  const [colors, setColors] = useState<string[]>(['#FF6B6B', '#FFD600', '#2979FF']); // Red, Yellow, Blue
  ...
  // Reset picker position when opening a new picker
  React.useEffect(() => {
  ...
  const handleAddColor = () => {
    ...
    setColorInput('#00C853'); // Reset to green after adding
  };
        ...
        <form className="w-full max-w-xs flex flex-col gap-4" onSubmit={e => { e.preventDefault(); handleCreate(); }}>
          ...
          <div>
            ...
            <div className="flex gap-2 mt-1">
              <button
                type="button"
                className={`flex-1 rounded-xl px-4 py-2 border border-white/20 ${type === 'numbers' ? 'bg-white/20 text-white font-bold' : 'bg-white/10 text-white/70'}`}
              ...
              <button
                type="button"
                className={`flex-1 rounded-xl px-4 py-2 border border-white/20 ${type === 'colors' ? 'bg-white/20 text-white font-bold' : 'bg-white/10 text-white/70'}`}
              ...
              <div className="flex flex-col gap-2 mt-2">
                {colors.map((color, idx) => (
                  <div key={idx} className="flex items-center gap-3 relative">
                    ...
                    />
                    <span className="text-white/80 font-mono">{color}</span>
                    <button type="button" className="ml-auto text-red-400 hover:text-red-300 text-xs" onClick={() => handleRemoveColor(color)}>Remove</button>
                    {showColorPicker && editingColorIdx === idx && (
                        <div
                          className="absolute left-0 top-full z-50 bg-white/10 rounded-xl p-3 shadow-lg border border-white/20 flex flex-col items-center"
                          style={{ minWidth: 180 }}
                        >
                          <HexColorPicker
                            color={colorInput}
                ...
                <div className="flex items-center gap-3 mt-2">
                  ...
                  <button
                    type="button"
                    className="rounded-lg px-3 py-1 bg-white/10 text-white/80 border border-white/20"
                ...
                {showColorPicker && editingColorIdx === null && (
                    <div className="mt-2"><HexColorPicker color={colorInput} onChange={setColorInput} /></div>
                  )}
              </div>
            </div>
          )}
          <div className="flex gap-4 mt-4">
            <button type="submit" className="flex-1 rounded-xl px-4 py-2 bg-white/20 text-white font-bold border border-white/20 hover:bg-white/30 transition">Create Dice</button>
            <button type="button" className="flex-1 rounded-xl px-4 py-2 bg-white/10 text-white/70 border border-white/20 hover:bg-white/20 transition" onClick={onClose}>Cancel</button>
          </div>
        </form>
      </div>
    </div>
